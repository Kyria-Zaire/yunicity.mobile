    ...
    async function load() {
      try {
        const res = await fetch(
          `${API_BASE}/ai/ai/recommendations?userId=${userId}&city=reims`,
          { credentials: 'include' },
        );
        if (res.ok) {
  ...
  return (
    <main className="min-h-screen bg-[#F9FAFB]">
      ...
      <section className="bg-white border-b px-4 sm:px-6 py-6">
        <h1 className="font-display font-bold text-[28px] text-[#0D0F2E]">
          Bonjour {'\u{1F44B}'}
        </h1>
        <p className="font-mono text-[12px] text-[#9395FF] mt-1">
          Mercredi 18 mars 2026 · Reims
        </p>
      </section>
      ...
      <section className="px-4 mt-6">
        ...
        <p className="font-mono text-[10px] text-[#9CA3AF] mt-1">
          source: {yuData.source} — les recommandations IA arrivent
          avec Yuni AI
        </p>
          ...
          {MOCK_POSTS.map((post) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="font-body text-[14px] text-[#374151] line-clamp-1">
                  {post.text}
                </p>
              </div>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 gap-4 px-4 mt-8 pb-12">
        ...
        <div className="bg-gradient-to-br from-[#2A2FFF] to-[#1A1ECC] rounded-2xl p-5">
          ...
          <p className="font-display font-bold text-[28px] text-white mt-1">
            340 pts
          </p>
          ...
          <p className="font-body text-[13px] text-white/70 mt-2">
            360 pts pour devenir Acteur
          </p>
          <Link
            href="/passport"
