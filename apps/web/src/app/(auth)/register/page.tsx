...
const PROFILES = [
  { id: 'yunicitizen' as const, label: 'Yunicitizen', desc: 'Habitant, curieux du quartier et de la ville.', emoji: '🏙️', ring: 'ring-[#2A2FFF]', circle: 'bg-[#2A2FFF]/15', badge: 'bg-[#E8E9FF] text-[#2A2FFF]' },
  { id: 'commercial' as const, label: 'Commerce', desc: 'Commerce, restauration, services locaux.', emoji: '🏪', ring: 'ring-[#16A34A]', circle: 'bg-[#16A34A]/15', badge: 'bg-[#DCFCE7] text-[#16A34A]' },
  { id: 'association' as const, label: 'Association', desc: 'Association loi 1901, culture, sport.', emoji: '🤝', ring: 'ring-[#D97706]', circle: 'bg-[#D97706]/15', badge: 'bg-[#FEF3C7] text-[#D97706]' },
  { id: 'freelance' as const, label: 'Freelance', desc: 'Indépendant, auto-entrepreneur, consultant.', emoji: '💼', ring: 'ring-[#7C3AED]', circle: 'bg-[#7C3AED]/15', badge: 'bg-[#F5F3FF] text-[#7C3AED]' },
  { id: 'ecole' as const, label: 'École / Entité', desc: 'Établissement scolaire ou structure éducative.', emoji: '🎓', ring: 'ring-[#059669]', circle: 'bg-[#059669]/15', badge: 'bg-[#ECFDF5] text-[#059669]' },
];
...
function passwordStrength(pw: string): { n: number; label: string } {
  const L = pw.length;
  if (L === 0) return { n: 0, label: '' };
  if (L < 8) return { n: 1, label: 'Faible' };
  if (L < 12) return { n: 2, label: 'Moyen' };
  ...
  if (step === 1) {
    return (
      <div>
        <div className="flex gap-1 mb-2">
          {[0, 1, 2].map((i) => (
            <div key={i} className="flex-1 flex flex-col gap-2">
              <div
                className={`h-1.5 rounded-full transition-all duration-300 ease-out ${i === 0 ? 'bg-[#2A2FFF]' : 'bg-[#F3F4F6]'}`}
              />
        ...
        <h1 className="font-display text-[28px] font-bold text-[#0D0F2E] mt-6">Qui es-tu sur Yunicity ?</h1>
        <p className="font-body text-base text-[#6B7280] mt-2">Choisis le profil qui te correspond — tu pourras le faire vérifier ensuite.</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-8">
          {PROFILES.map((p) => {
            ...
            return (
              ...
              >
                {sel && (
                  <span className="absolute top-3 right-3 w-7 h-7 rounded-full bg-[#2A2FFF] text-white flex items-center justify-center text-sm font-bold animate-scale-in" aria-hidden>
                    ✓
                  </span>
                )}
                <div className={`w-12 h-12 rounded-full ${p.circle} flex items-center justify-center text-2xl mb-3`}>
                  {p.emoji}
                </div>
                ...
                <span className={`inline-block mt-3 font-mono text-[10px] uppercase tracking-wider px-2.5 py-0.5 rounded-full ${p.badge}`}>Profil</span>
              </button>
            );
          })}
        </div>
        ...
        <div className="flex gap-1 mb-4">
          {[0, 1, 2].map((i) => (
            <div key={i} className="flex-1">
              <div
                className={`h-1.5 rounded-full transition-all duration-300 ${i <= 1 ? 'bg-[#2A2FFF]' : i === 2 ? 'bg-[#2A2FFF]/40' : 'bg-[#F3F4F6]'}`}
              />
            </div>
          ))}
        </div>
        <button type="button" onClick={() => setStep(1)} className="text-sm text-[#6B7280] hover:text-[#2A2FFF] mb-4">
          ← Changer de profil
        </button>
          ...
          <div>
            ...
            <div className={`${inputWrap} mt-1.5`}>
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#9CA3AF]">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 4h16v16H4z M22 6l-10 7L2 6" /></svg>
              </span>
              <input {...step2.register('email')} type="email" className={`${inputClass} ${step2.formState.errors.email ? 'border-[#DC2626]' : ''}`} placeholder="toi@exemple.fr" />
            </div>
            {step2.formState.errors.email && <p className="text-[#DC2626] text-sm mt-1">{step2.formState.errors.email.message}</p>}
          </div>
          <div>
            ...
            <div className={`${inputWrap} mt-1.5`}>
              ...
              />
              {strength.n === 4 && (
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[#16A34A]" aria-hidden>✓</span>
              )}
            </div>
            ...
            {strength.label && <p className="text-xs text-[#6B7280] mt-1">Robustesse : {strength.label}</p>}
            {step2.formState.errors.password && <p className="text-[#DC2626] text-sm mt-1">{step2.formState.errors.password.message}</p>}
          </div>
          <div>
            <label className="font-body text-[13px] font-medium text-[#374151]">Téléphone <span className="text-[#9CA3AF] font-normal">(optionnel)</span></label>
            <div className={`${inputWrap} mt-1.5`}>
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#9CA3AF]">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72 12.84 12.84 0 00.7 2.81 2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45 12.84 12.84 0 002.81.7A2 2 0 0122 16.92z" /></svg>
              </span>
              <input {...step2.register('phone')} type="tel" className={inputClass} placeholder="+33612345678" />
            </div>
            {step2.formState.errors.phone && <p className="text-[#DC2626] text-sm mt-1">{step2.formState.errors.phone.message}</p>}
          </div>
          {(profile === 'commercial' || profile === 'freelance') && (
            <div className="rounded-xl border border-dashed border-[#E5E7EB] bg-[#F9FAFB] p-4">
              <p className="text-xs italic text-[#9CA3AF] mb-2">Requis pour la vérification de votre profil</p>
              <label className="font-body text-[13px] font-medium text-[#374151]">SIRET</label>
              <input {...step2.register('siret')} className="w-full h-11 mt-1 rounded-lg border border-[#D1D5DB] px-3 text-sm" placeholder="14 chiffres" />
              <p className="text-xs text-[#6B7280] mt-2">Votre SIRET sera vérifié automatiquement via l&apos;INSEE.</p>
            </div>
          )}
          {profile === 'association' && (
            <div className="rounded-xl border border-dashed border-[#E5E7EB] bg-[#F9FAFB] p-4">
              ...
              <input {...step2.register('rna')} className="w-full h-11 mt-1 rounded-lg border border-[#D1D5DB] px-3 text-sm" placeholder="W123456789" />
            </div>
          )}
          {profile === 'ecole' && (
            <div className="rounded-xl border border-dashed border-[#E5E7EB] bg-[#F9FAFB] p-4">
              ...
              <input {...step2.register('uai')} className="w-full h-11 mt-1 rounded-lg border border-[#D1D5DB] px-3 text-sm" placeholder="0123456A" />
            </div>
          )}
          <button type="submit" className="w-full h-12 rounded-xl bg-[#2A2FFF] text-white font-semibold shadow-primary hover:bg-[#1A1ECC] transition-all">
            Continuer →
          </button>
        </form>
      </div>
  ...
  return (
    <div>
      <div className="flex gap-1 mb-6">
        {[0, 1, 2].map((i) => (
          <div key={i} className="flex-1 h-1.5 rounded-full bg-[#2A2FFF]/60" />
        ))}
      </div>
      ...
      <h1 className="font-display text-2xl font-bold text-[#0D0F2E] text-center">Vos données, vos droits.</h1>
      <p className="font-body text-sm text-[#6B7280] text-center mt-3 leading-relaxed">
        Yunicity traite vos données pour créer et sécuriser votre compte, conformément au RGPD. Vous disposez d&apos;un droit d&apos;accès, de rectification et d&apos;effacement. Données hébergées en France.
      </p>
      <form
        className="mt-8 space-y-5"
        onSubmit={step3.handleSubmit(async () => {
          ...
          try {
            await signUp.email({
              ...
              callbackURL: '/verification-pending',
            });
        ...
        <div className="space-y-4">
          <label className="flex items-start gap-3 cursor-pointer">
            ...
            <span className="mt-0.5 w-5 h-5 shrink-0 rounded-md border-2 border-[#D1D5DB] flex items-center justify-center peer-checked:bg-[#2A2FFF] peer-checked:border-[#2A2FFF] peer-focus-visible:ring-2 peer-focus-visible:ring-[#2A2FFF] peer-checked:[&_svg]:opacity-100 transition-colors">
              <svg className="w-3 h-3 text-white opacity-0 transition-opacity" viewBox="0 0 12 10" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M1 5l3 3 7-7" />
              </svg>
            </span>
            <span className="font-body text-sm text-[#374151] leading-relaxed">
              J&apos;accepte le traitement de mes données personnelles selon la{' '}
              <a href="/privacy" className="text-[#2A2FFF] underline">politique de confidentialité</a>
              . <span className="text-[#DC2626]">*</span>
            </span>
          </label>
          {step3.formState.errors.rgpd && <p className="text-[#DC2626] text-sm">{step3.formState.errors.rgpd.message}</p>}
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            ...
            <span className="font-body text-sm text-[#6B7280]">J&apos;accepte de recevoir des actualités Yunicity (optionnel).</span>
          </label>
        </div>

        {error && (
          <div className="rounded-xl bg-[#FEE2E2] border border-[#DC2626]/30 px-4 py-3 text-sm text-[#DC2626]">{error}</div>
        )}
